...
| **HR Helpdesk / case management** ⭐ | Employees raise HR/payroll/IT tickets; HR queue with assign, status flow, threaded replies + notifications | `helpdesk` |
...
1. **Offboarding / exit** — resignation, notice period, clearance checklist, full-and-final.
2. **Payroll compliance (India)** — PF / ESI / PT / TDS computations and statutory reports.
3. **Asset management** — assign laptops/devices to employees, track returns.
4. **Timesheets** — project/task time, billable vs non-billable (Keka project timesheets).
5. **Learning & Development (LMS)** — courses, assignments, completion tracking (Zoho add-on).
6. **Surveys & engagement** — pulse surveys, eNPS.
7. **Rewards & recognition** — kudos, points, badges.
8. **Advanced attendance** — geofencing, biometric/kiosk, mobile punch-in.
9. **Resume parsing & offer workflows** — deepen the ATS toward Keka parity.
