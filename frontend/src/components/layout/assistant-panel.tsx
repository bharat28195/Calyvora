...
export function AssistantPanel() {
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed bottom-6 right-6 z-40 flex h-[min(560px,80vh)] w-[min(420px,92vw)] flex-col overflow-hidden rounded-2xl border border-fg/10 bg-surface shadow-2xl">
          <div className="flex items-center justify-between border-b border-fg/10 px-4 py-3">
            ...
            <button onClick={() => setOpen(false)} className="text-fg/40 hover:text-fg" aria-label="Close">
              <X className="h-4 w-4" />
            </button>
          </div>
                ...
                <div className="mt-4 flex flex-col gap-2">
                  {SUGGESTIONS.map((s) => (
                    <button key={s} onClick={() => send(s)}
                      className="rounded-lg border border-fg/10 bg-fg/5 px-3 py-2 text-left text-sm text-fg/80 hover:bg-fg/10">
                      {s}
                    </button>
                  ))}
                </div>
            ...
            {turns.map((turn, i) => {
              ...
              return (
                <div key={i} className="flex flex-col gap-2">
                  <div className="max-w-[90%] rounded-2xl rounded-bl-sm border border-fg/10 bg-fg/[0.03] px-3 py-2 text-sm">
                    <Markdown text={turn.answer} />
                    {!isError && "mode" in turn && (
                      <div className="mt-2 flex items-center gap-1.5 text-[10px] text-fg/30">
                        <span className={`rounded-full px-1.5 py-0.5 ${turn.mode === "claude" ? "bg-violet/20 text-violet" : "bg-fg/10"}`}>
          ...
          <form
            ...
            className="flex items-center gap-2 border-t border-fg/10 p-3"
          >
            <input
              value={q}
              onChange={(e) => setQ(e.target.value)}
              placeholder="Ask about your company…"
              className="h-10 flex-1 rounded-lg border border-fg/10 bg-fg/5 px-3 text-sm text-fg placeholder:text-fg/30 focus:border-violet focus:outline-none"
            />
        ...
        <p key={i}>
          {line.split(/(\*\*[^*]+\*\*)/g).map((part, j) =>
            part.startsWith("**") && part.endsWith("**") ? (
              <strong key={j} className="font-semibold text-fg">{part.slice(2, -2)}</strong>
            ) : (
