    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-white/50">Open tasks assigned to you across all projects.</p>
        </div>
        <Link href="/work" className="text-sm text-white/60 hover:text-white">← Projects</Link>
      </div>
      ...
      {tasks === null ? (
        ...
        <Card className="mt-8 py-12 text-center">
          ...
          <p className="mt-1 text-sm text-white/50">No open tasks are assigned to you.</p>
        </Card>
      ) : (
        <div className="mt-8 flex flex-col gap-2">
          {tasks.map((t) => (
            <Link key={t.id} href={`/work/${t.projectId}`}>
              <Card className="flex items-center justify-between p-4 transition-colors hover:border-white/20">
                ...
                <div className="flex items-center gap-3 text-xs text-white/40">
                  ...
                  <span className="text-white/50">{statusLabel[t.status]}</span>
