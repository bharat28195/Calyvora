...
public record CompanySettingsResponse(String companyId, String timezone, String locale, String logoUrl) {
    ...
    public static CompanySettingsResponse of(CompanySettings settings) {
        return new CompanySettingsResponse(settings.getCompanyId().toString(),
                settings.getTimezone(), settings.getLocale(), settings.getLogoUrl());
    }
}
